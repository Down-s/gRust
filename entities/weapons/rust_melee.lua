...
function SWEP:PrimaryAttack()
    ...
    if self.m_AttackBlockTime > CurTime() then
        return
    end

    if (self.IsThrowing) then
        if self.m_NextThrow == 0 then
            self.m_NextThrow = CurTime() + 0.275
    ...
    if self.m_NextAttack > CurTime() then
        return
    end
    ...
    self.m_NextAttack = CurTime() + self.AttackDelay
...
function SWEP:CheckThrow()
    ...
    if (pl:KeyDown(IN_ATTACK2)) then
        if (!self.IsThrowing and !self.Thrown and self.m_NextAttack == 0) then
    ...
    if self.m_NextThrow != 0 and self.m_NextThrow <= CurTime() and self.IsThrowing and !self.Thrown then
        self:Throw()
    end
end
...
function SWEP:Think()
    ...
    if self.m_NextAttack == 0 or self.m_NextAttack > CurTime() then
        return
    end
