...
function SWEP:Throw()
    if not IsValid(self:GetItem()) then return end
    ...
    if SERVER then
        if not pl.Belt[self:GetBeltIndex()] then return end
        ...
        if IsValid(phys) then phys:SetVelocity(pl:EyeAngles():Forward() * 1500) end
    end
end
...
function SWEP:Hit(tr)
    ...
    if IsValid(ent) then
        ...
        if SERVER then
            if ent:IsPlayer() then hook.Run("ScalePlayerDamage", ent, tr.HitGroup, dmg) end
            ent:TakeDamageInfo(dmg)
        end
    end

    if SERVER then
        self:LoseCondition(0.050)
...
function SWEP:CheckThrow()
    ...
    if pl:KeyDown(IN_ATTACK2) then
        ...
        if not self.IsThrowing and not self.Thrown and nextAttack == 0 then
    ...
    if self:GetNextThrow() ~= 0 and self:GetNextThrow() <= CurTime() and self.IsThrowing and not self.Thrown then self:Throw() end
end
