...
function ENT:CreateExtraOptions()
    if self.LockBodygroup then
        ...
        PieMenu:CreateOption():SetTitle("Unlock"):SetDescription("Unlock this lock"):SetIcon("unlock"):SetCondition(true, function()
            if not IsValid(self) then return false end
            return self:GetBodygroup(self.LockBodygroup) ~= 0 and self:GetSkin() == 1
        ...
        PieMenu:CreateOption():SetTitle("Lock"):SetDescription("Lock this lock"):SetIcon("lock"):SetCondition(true, function()
            if not IsValid(self) then return false end
            return self:GetBodygroup(self.LockBodygroup) ~= 0 and self:GetSkin() == 0
        end):SetCallback(function()
            net.Start("gRust.ToggleLock")
            net.WriteEntity(self)
            net.SendToServer()
        ...
        PieMenu:CreateOption():SetTitle("Remove Lock"):SetDescription("Remove this lock and place it in your inventory"):SetIcon("give"):SetCondition(true, function()
            if not IsValid(self) then return false end
            return self:GetBodygroup(self.LockBodygroup) ~= 0 and self:GetSkin() == 0
        end):SetCallback(function()
        ...
        PieMenu:CreateOption():SetTitle("Create Key"):SetDescription("Craft a key for this lock to allow friends to enter"):SetFooter(function() return string.format("25 x Wood (%s)", string.Comma(LocalPlayer():ItemCount("wood"))) end):SetIcon("key"):SetCondition(true, function()
            if not IsValid(self) then return false end
            return self:GetBodygroup(self.LockBodygroup) == 2
        end):SetCallback(function() end)

        PieMenu:CreateOption():SetTitle("Change Lock Code"):SetDescription("Change the code to something else"):SetIcon("changecode"):SetCondition(true, function()
            if not IsValid(self) then return false end
        ...
        PieMenu:CreateOption():SetTitle("Unlock With Code"):SetDescription("This lock has a pass code which you must neter to unlock it"):SetIcon("unlock"):SetCondition(true, function()
            if not IsValid(self) then return false end
            return self:GetBodygroup(self.LockBodygroup) == 1
        end):SetCallback(function()
...
function ENT:Initialize()
    if CLIENT then
        ...
        self:SetupSockets()
        if self.MaxHP and not self:CreatedByMap() then
...
function ENT:OnTakeDamage(dmg)
    ...
    if IsValid(attacker) and attacker:IsPlayer() then attacker:EmitSound("combat.hitmarker") end
    local scale = 1
...
function ENT:Lock()
    if not self.LockData then return end
    self:SetSkin(1)
end
...
function ENT:IsLocked()
    if not self.LockData then return false end
    return self:GetSkin() == 1
end

function ENT:ToggleLock(pl)
    if not self.LockData then return end
    if not self:IsAuthorizedToLock(pl) then return end
...
function ENT:IsAuthorizedToLock(pl)
    if not self.LockData then return true end
...
function ENT:CreateRespawn()
    if CLIENT then return end
    if not self.RespawnTime then return end
...
--
if SERVER then
    local BUILD_RADIUS = gRust.GetConfigValue("building/tool_cupboard.radius") ^ 2 or 0
    ...
    function ENT:DecayThink()
        ...
        if self.Decay and self.NextDecayThink <= CurTime() then
            ...
            if not IsValid(tc) or tc:GetPos():DistToSqr(self:GetPos()) > BUILD_RADIUS then self:DoDecay(DECAY_THINK_DELAY) end
        end
    end
...
function ENT:Save(f)
    ...
    if self.MaxHP then f:WriteFloat(self:GetHP()) end
    if self.LockData then
        ...
        if self.LockData.Code then f:WriteShort(self.LockData.Code) end
    else
        f:WriteBool(false)
    end
...
function ENT:Load(f)
    ...
    if self.MaxHP then self:SetHP(f:ReadFloat()) end
    if f:ReadBool() then
        ...
        if f:ReadBool() then self.LockData.Code = f:ReadShort() end
    end

    if f:ReadBool() then self.OwnerID = f:ReadString() end
end
...
if SERVER then
    ...
    net.Receive("gRust.ToggleLock", function(len, pl)
        ...
        if not ent:GetInteractable() then return end
        if pl:IsBuildBlocked() then return end
        if not ent:IsAuthorizedToLock(pl) then return end
        ent:ToggleLock(pl)
    end)
