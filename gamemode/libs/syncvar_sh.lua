...
if SERVER then
    ...
    function PLAYER:RegisterSyncVar(name, type, bits)
        ...
        if self:IsNetworkReady() then
            ...
            if type == SyncVar.Int or type == SyncVar.UInt then net.WriteUInt(bits, 5) end
            net.Send(self)
        else
    ...
    hook.Add("PlayerNetworkReady", "gRust.SyncVars", function(pl)
        ...
        if queue then
            ...
            for i = 1, #queue do
                ...
                if syncVar.type == SyncVar.Int or syncVar.type == SyncVar.UInt then net.WriteUInt(syncVar.bits, 5) end
                net.WriteBit(syncVar.value ~= nil)
                if syncVar.value ~= nil then WRITE_FUNCTIONS[syncVar.type](syncVar.value, syncVar.bits) end
            end
    ...
    function PLAYER:SetSyncVar(name, value)
        ...
        if type == SyncVar.Int or type == SyncVar.UInt then
            ...
            if oldValue == newValue then shouldNetwork = false end
        end

        if syncVar then
            local name2 = syncVar[1]
            self.SyncVarsRegister = self.SyncVarsRegister or {}
            local index2 = #self.SyncVarsRegister + 1
            self.SyncVarsRegister[index2] = {name2, type, bits}
            self.SyncVarBits = math.ceil(math.log(#self.SyncVarsRegister, 2))
            local oldValue = self.SyncVars[name2]
            ...
            if proxy then proxy(pl, name, oldValue, value) end
        end
    end
else
    --
    ...
    net.Receive("gRust.RegisterSyncVar", function(len)
        ...
        local bits = 32
        if type == SyncVar.Int or type == SyncVar.UInt then bits = net.ReadUInt(5) end
        self.SyncVars = self.SyncVars or {}
        self.SyncVarsIndex = self.SyncVarsIndex or {}
        self.SyncVarsRegister = self.SyncVarsRegister or {}
    ...
    net.Receive("gRust.CreateSyncVars", function(len)
        ...
        for i = 1, count do
            ...
            local bits = 32
            if type == SyncVar.Int or type == SyncVar.UInt then bits = net.ReadUInt(5) end
            if net.ReadBit() == 1 then pl.SyncVars[name] = READ_FUNCTIONS[type](bits) end
            pl.SyncVars = pl.SyncVars or {}
            pl.SyncVarsIndex = pl.SyncVarsIndex or {}
            pl.SyncVarsRegister = pl.SyncVarsRegister or {}
    ...
    net.Receive("gRust.SyncVar", function(len)
        local pl = LocalPlayer()
        local index = net.ReadUInt(pl.SyncVarBits or 100)
        if not pl.SyncVarsRegister then return end
...
function PLAYER:GetSyncVar(name, default)
    if not self.SyncVars then return default end
