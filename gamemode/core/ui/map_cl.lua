...
local function ReloadVendingMachines()
    ...
    for i = 1, #vendingMachines do
        ...
        if not vm:GetBroadcasting() then continue end
        local pos = vm:GetPos()
...
hook.Add("PlayerButtonDown", "gRust.OpenMap", function(pl, key)
    if MapOpen and key == 107 then
        local x, y = gRust.Map:CursorPos()
        local worldPos = gRust.MapToPos(x,y)
...
hook.Add("Think", "gRust.MapToggle", function()
    if input.IsKeyDown(KEY_G) and not MapOpen and not vgui.CursorVisible() and gRust.Hud.ShouldDraw then
        gui.EnableScreenClicker(true)
        MapOpen = true
        MapOpenTime = CurTime()
        ReloadVendingMachines()
    elseif not input.IsKeyDown(KEY_G) and MapOpen then
...
local function DrawPlayers()
    ...
    if gRust.TeamsIndex then
        for _, other in player.Iterator() do
            ...
            if not gRust.TeamsIndex[other:SteamID64()] then continue end
...
local function DrawOres()
    ...
    for k, v in pairs(ents.FindByClass("rust_ore")) do
        if v:GetSkin() == 0 then
            local sorx, sory = gRust.PosToMap(v:GetPos())
            surface.SetDrawColor(255, 255, 255, 255)
            surface.SetMaterial(Rust_Ores_Stone)
            surface.DrawTexturedRectRotated(sorx, sory, 20 * scaling, 20 * scaling, 90, 0, 0)
        end

        if v:GetSkin() == 1 then
            local sorxx, soryy = gRust.PosToMap(v:GetPos())
            surface.SetDrawColor(255, 255, 255, 255)
            surface.SetMaterial(Rust_Ores_Metal)
            surface.DrawTexturedRectRotated(sorxx, soryy, 20 * scaling, 20 * scaling, 90, 0, 0)
        end

        if v:GetSkin() == 2 then
            local sorxxx, soryyy = gRust.PosToMap(v:GetPos())
            surface.SetDrawColor(255, 255, 255, 255)
            surface.SetMaterial(Rust_Ores_Sulfur)
            surface.DrawTexturedRectRotated(sorxxx, soryyy, 20 * scaling, 20 * scaling, 90, 0, 0)
...
local function CreateMap()
    if IsValid(gRust.Map) then gRust.Map:Remove() end
