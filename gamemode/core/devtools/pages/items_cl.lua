...
function PANEL:Init()
    ...
    for k, v in ipairs(gRust.GetCategories()) do
        ...
        if #v.Items == 0 then Category:SetTextColor(Color(255, 255, 255, 25)) end
        if v.Name == OpenedTab then self:FillCategory(v.Name) end
    ...
    Search.OnChange = function(me) self:FillItems(function(item) return string.find(string.lower(item:GetName()), string.lower(me:GetText())) end) end
    Search.OnEnter = function(me)
        local FirstItem = self.Container:GetChildren()[1]
        if IsValid(FirstItem) then FirstItem:DoClick() end
        me:RequestFocus()
    end
...
function PANEL:FillItems(condition)
    ...
    for k, v in ipairs(gRust.GetItems()) do
        ...
        if not condition(register) then continue end
        ...
        Item.DoClick = function(me) SpawnItem(v, 1) end
        ...
        Hundred.DoClick = function(me) SpawnItem(v, 100) end
        ...
        Thousand.DoClick = function(me) SpawnItem(v, 1000) end
        ...
        Hundred.Paint = function(me, w, h) if Item:IsHovered() or me:IsHovered() or Thousand:IsHovered() then OldPaint(me, w, h) end end
        local OldPaint = Thousand.Paint
        Thousand.Paint = function(me, w, h) if Item:IsHovered() or me:IsHovered() or Hundred:IsHovered() then OldPaint(me, w, h) end end
    end
...
function PANEL:FillCategory(category)
    self:FillItems(function(item) return item:IsInCategory(category) end)
    return true
end
