...
net.Receive("gRust.SwapInventorySlots", function(len, pl)
    if pl.CoolDownDropITem == nil then pl.CoolDownDropITem = 0 end
    pl.CoolDownDropITem = CurTime() + 1
...
net.Receive("gRust.DropItem", function(len, pl)
    ...
    if not inventory or not inventory:CanPlayerAccess(pl) then return end
    ...
    timer.Simple(0, function()
        ...
        if IsValid(phys) then phys:SetVelocity(pl:GetAimVector() * 200) end
    end)
