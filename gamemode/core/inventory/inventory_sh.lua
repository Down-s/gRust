...
function INVENTORY:Copy()
    ...
    for i = 1, self.Slots do
        if not IsValid(self[i]) then continue end
...
function INVENTORY:CanPlayerAccess(pl)
    if not self:CustomCheck(pl) then return false, "You are not authorized to access this container" end
    if not pl:Alive() then return false, "You are not authorized to access this container" end
    ...
    for i = 1, #players do
        if players[i] == pl then
            if IsValid(ent) and ent ~= pl then
                if ent:GetPos():DistToSqr(pl:GetPos()) > 20000 then return false, "Too far away" end
                ...
                if IsValid(tr.Entity) and tr.Entity ~= ent then return false, "Line of sight blocked" end
            end
            return true
        end
    end
...
function INVENTORY:HasItem(item, quantity)
    ...
    for i = 1, self.Slots do
        if self[i]:GetId() == item:GetId() then return true end
    end
    return false
end
...
function INVENTORY:Set(i, item)
    self[i] = item
    self:OnUpdated()
    if SERVER then self:SyncSlot(i) end
end

function INVENTORY:Remove(i, quantity)
    if i < 1 or i > self.Slots then return end
    local item = self[i]
    if not quantity or (quantity >= item:GetQuantity()) then
        self[i] = nil
    else
...
function INVENTORY:Clear()
    ...
    if SERVER then self:SyncAll() end
end
...
function INVENTORY:SwapSlot(index, other, otherIndex, quantity)
    ...
    if not IsValid(item) and not IsValid(otherItem) then return end
    ...
    if quantity == 0 or quantity > item:GetQuantity() then return end
    local combineable = not otherItem or (item:GetId() == otherItem:GetId() and (item:GetQuantity() + otherItem:GetQuantity()) <= itemRegister:GetStack())
    if IsValid(otherItem) and not combineable and not self:CanAcceptItem(otherItem, other) then return end
    if not other:CanAcceptItem(item, self) then return end
    if not self:CanRemoveItem(item, other) then return end
    if IsValid(otherItem) then
        if otherItemRegister:IsWeapon() then
            ...
            if data and data.Attachments and data.Attachments[item:GetId()] then
                for i = 1, otherItemRegister:GetSlots() or 0 do
                    ...
                    if not data.Attachments[slot:GetId()] then continue end
                    if data.Attachments[item:GetId()].Type == data.Attachments[slot:GetId()].Type then return end
                end
        ...
        if item:GetId() == otherItem:GetId() then
            ...
            if item:GetQuantity() <= 0 then self[index] = nil end
            self:OnUpdated()
            other:OnUpdated()
            self:SyncSlot(index)
            other:SyncSlot(otherIndex)
...
function INVENTORY:ItemCount(item)
    ...
    for i = 1, self.Slots do
        if not IsValid(self[i]) then continue end
        local slot = self[i]
        if not item or (slot:GetId() == item) then count = count + slot:GetQuantity() end
    end
    return count
end
...
function INVENTORY:FirstEmpty()
    for i = 1, self.Slots do
        if self[i] == nil then return i end
    end
    return nil
end
...
function INVENTORY:GetViableSlot(item, container)
    ...
    for i = 1, self:GetSize() do
        local slot = self[i]
        if not self:CanAcceptItem(item, container) then continue end
        if not IsValid(slot) then
            table.insert(viableSlots, i)
        elseif IsValid(slot) and slot:GetId() == item:GetId() then
            if slot:GetQuantity() < slot:GetRegister():GetStack() then table.insert(viableSlots, i) end
        end
    end
...
function INVENTORY:CanStackItem(item, container)
    ...
    for i = 1, self:GetSize() do
        local slot = self[i]
        if IsValid(slot) and slot:GetId() == item:GetId() then if (slot:GetQuantity() + item:GetQuantity()) <= slot:GetRegister():GetStack() then return true end end
    end
    return false
end
...
function INVENTORY:CanInsertItem(item, container)
    if not item then return false end
    if not self:CanAcceptItem(item, container) then return false end
    ...
    for i = 1, self:GetSize() do
        ...
        if slot == nil then
            return true
        elseif IsValid(slot) and slot:GetId() == item:GetId() then
            if slot:GetQuantity() < slot:GetRegister():GetStack() then remaining = remaining - (slot:GetRegister():GetStack() - slot:GetQuantity()) end
        end
    end
...
function INVENTORY:InsertItem(item, noempty)
    ...
    for i = 1, self:GetSize() do
        local slot = self[i]
        if slot == nil and not noempty then
            table.insert(useSlots, i)
        elseif IsValid(slot) and slot:GetId() == item:GetId() then
            if slot:GetQuantity() < slot:GetRegister():GetStack() then table.insert(useSlots, i) end
        end
    end
...
function INVENTORY:AddReplicatedPlayer(pl, sync)
    if not self.ReplicatedPlayers[pl] then
    ...
    if SERVER then
        if sync then
            ...
            if IsValid(self.Entity) then net.WriteEntity(self.Entity) end
            net.Send(pl)
        else
...
function INVENTORY:SetEntity(ent)
    self.Entity = ent
    if ent ~= nil then
        if ent.OnInventoryAttached then ent:OnInventoryAttached(self) end
        if SERVER then
...
function INVENTORY:SyncAll(pl)
    ...
    if self.Entity then net.WriteEntity(self.Entity) end
...
function gRust.CreateInventory(slots, index, ent)
    ...
    if IsValid(ent) then inventory:SetEntity(ent) end
