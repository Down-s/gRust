...
if SERVER then gRust.CreateConfigValue("farming/stack.multiplier", 1, true) end
function net.WriteItem(item)
    local index = IsValid(item) and item:GetIndex() or 0
    net.WriteUInt(index, gRust.ItemIndexBits)
    if index == 0 or not IsValid(item) then return end
    local register = item:GetRegister()
    if register:GetStack() > 1 then net.WriteUInt(item:GetQuantity(), register.StackBits) end
...
function net.ReadItem()
    ...
    if slots > 0 then
        ...
        if not gRust.Inventories[inventoryIndex] then gRust.CreateInventory(slots, inventoryIndex) end
        for i = 1, slots do
...
function FILE:WriteItem(item)
    ...
    if index == 0 or not IsValid(item) then return end
    local register = item:GetRegister()
    if register:GetStack() > 1 then self:WriteULong(item:GetQuantity()) end
    if item:IsWeapon() then self:WriteByte(item:GetClip() or 0) end
    if register:GetCondition() then
...
function FILE:ReadItem()
    ...
    if Item:IsWeapon() then Item:SetClip(self:ReadByte()) end
    if Item:GetRegister():GetCondition() then
...
ITEM.__index = ITEM
ITEM.__tostring = function(self) return string.format("item: %s [x%i]", self:GetRegister():GetName(), self:GetQuantity()) end
...
function ITEM:Split(n)
    ...
    if self:GetInventory() then item:SetInventory(self:GetInventory()) end
    return item
end
...
function gRust.CreateItem(id, quantity)
    ...
    if not register then error(string_format("Attempted to create an item with an invalid id '%s'", id)) end
    ...
    if slots and slots > 0 then
        ...
        meta.Inventory.OnUpdated = function(self) if meta.ParentInventory then meta.ParentInventory:SyncSlot(meta.ParentSlot) end end
        meta.Inventory.CanPlayerAccess = function(self, pl)
            if meta.ParentInventory then return meta.ParentInventory:CanPlayerAccess(pl) end
            ...
            if register:IsWeapon() then
                ...
                if item:GetRegister():IsInCategory("Attachments") then
                    ...
                    for i = 1, self:GetSlots() do
                        ...
                        if not slotRegister then continue end
                        local attachment = swep.Attachments and swep.Attachments[slotRegister:GetId()]
                        if not attachment then continue end
                        if attachment.Type == attachmentType then return false end
                    end
                    return true
                end
            end
...
function gRust.GetItemRegistered()
    return ItemRegistry
end
...
function ITEM_R:GetClipSize()
    if not self.ClipSize then
        local weapon = weapons.GetStored(self:GetWeapon())
        if weapon then self.ClipSize = weapon.ClipSize end
    end
...
function ITEM_R:IsBlueprint()
    return
        --string.EndsWith(self.Id, "_blueprint") and
...
function ITEM_R:AddToCategory(category)
    if not ItemCategories[ItemCategoryIndex[category]] then
...
if SERVER then
    ...
    net.Receive("gRust.ItemAction", function(len, pl)
        ...
        local register = item:GetRegister()
        if not register.Actions then return end
...
function gRust.RegisterItem(item)
    ...
    if name then
        item:AddAction(name, icon, function(pl, inventory, slot)
            ...
            if item:GetCalories() then pl:AddCalories(item:GetCalories()) end
            if item:GetHydration() then pl:AddHydration(item:GetHydration()) end
            if item:GetRadiation() then pl:AddRadiation(item:GetRadiation()) end
            if item:GetPoison() then pl:AddPoison(item:GetPoison()) end
            if item:GetHealing() then pl:AddHealing(item:GetHealing()) end
            if item:GetInstantHealth() then pl:SetHealth(pl:Health() + item:GetInstantHealth()) end
            if item:GetBleeding() then pl:AddBleeding(item:GetBleeding()) end
            inventory:Remove(slot, 1)
            if item:GetOnConsumed() then
                if not item:GetOnConsumed()(pl, inventory, slot) and snd then pl:EmitSound(snd) end
            elseif snd then
                pl:EmitSound(snd)
            end
        end)
    ...
    if math_pow(2, CurrentIndex + 1) > gRust.ItemIndexBits then gRust.ItemIndexBits = math_ceil(math_log(CurrentIndex + 1, 2)) end
    ...
    if item.ResearchCost then
        gRust.ItemRegister(item.Id .. "_blueprint"):SetName(item.Name .. " Blueprint"):SetDescription("A blueprint for " .. item.Name):SetStack(1):SetIcon(item.IconPath):SetMaterial("Paper"):AddToCategory("Blueprints"):AddAction("Learn Blueprint", "learn", function(pl, inventory, slot)
            local item = inventory[slot]
            if not item then return end
            local register = item:GetRegister()
            if not register then return end
            if not register:IsInCategory("Blueprints") then return end
    ...
    if item.Model then util.PrecacheModel(item.Model) end
    if item.Attire then util.PrecacheModel(item.Attire:GetModel()) end
end
...
hook.Add("gRust.ConfigInitialized", "gRust.UpdateItemStacks", function()
    ...
    for k, v in pairs(ItemRegistry) do
        ...
        if v:GetAbsoluteMax() then v:SetAbsoluteMax(v:GetAbsoluteMax() * multiplier) end
    end
end)