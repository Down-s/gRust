...
function gRust.Save(filename)
    ...
    if not file.Exists(saveFile, "DATA") then file.Write(saveFile, "") end
...
function gRust.ClearSave(filename)
    local saveFile = SAVE_DIR .. filename
    if file.Exists(saveFile, "DATA") then file.Delete(saveFile) end
end
...
hook.Add("PlayerSpawn", "SaveShutDowngRust", function() gRust.AutoSave() end)
hook.Add("PlayerDisconnected", "SaveShutDowngRust", function() gRust.AutoSave() end)
hook.Add("InitPostEntity", "gRust.Load", function() timer.Simple(4, function() gRust.Load("autosave.dat") end) end)
hook.Add("gRust.Wipe", "gRust.ClearAutoSave", function() gRust.ClearSave("autosave.dat") end)
...
concommand.Add("grust_load", function(pl, cmd, args)
    if IsValid(pl) and not pl:IsSuperAdmin() then return end
    for _, ent in ents.Iterator() do
        if ent.ShouldSave and not ent:CreatedByMap() then ent:Remove() end
    end
