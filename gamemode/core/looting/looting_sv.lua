...
function gRust.StartLooting(pl, ent)
    ...
    if not ent:IsLootable() then return end
    if not ent:CanPlayerAccess(pl) then return end
    if not ent.Containers then return end
    if IsValid(pl.LootingEntity) then return end
    if pl.LootingInventories then gRust.StopLooting(pl) end
    pl.LootingInventories = {}
    for k, v in ipairs(ent.Containers) do
        if v.AddReplicatedPlayer then v:AddReplicatedPlayer(pl, true) end
    ...
    timer.Create(timerName, 1, 0, function()
        ...
        if not ent:CanPlayerAccess(pl) then
            gRust.StopLooting(pl)
            timer.Remove(timerName)
            return
        end
...
function gRust.StopLooting(pl)
    if not pl.LootingInventories then return end
    for k, v in ipairs(pl.LootingInventories) do
        if v.RemoveReplicatedPlayer then v:RemoveReplicatedPlayer(pl) end
    end

    pl.LootingInventories = nil
    if IsValid(pl.LootingEntity) then
        if pl.LootingEntity.Looters then pl.LootingEntity.Looters[pl] = nil end
...
net.Receive("gRust.StopLooting", function(len, pl) gRust.StopLooting(pl) end)
hook.Add("PlayerDisconnected", "gRust.StopLooting", function(pl) gRust.StopLooting(pl) end)
hook.Add("PlayerDeath", "gRust.StopLooting", function(pl) gRust.StopLooting(pl) end)
--
-- Looting functions
--
function gRust.SelectLootFromTable(lootTable, amount)
    ...
    while #selectedLoot < amount do
        ...
        for _, v in ipairs(lootTable) do
            ...
            if not gRust.GetItemRegister(v.itemid) then continue end
            if randomChance <= v.chance then
                local quantity = v.amount
                if istable(quantity) then quantity = math.random(quantity[1], quantity[2]) end
                table.insert(selectedLoot, gRust.CreateItem(v.itemid, quantity))
                selectedLootIndex[v.itemid] = true
            end

            if #selectedLoot == #lootTable then break end
        end
    end
    return selectedLoot
end