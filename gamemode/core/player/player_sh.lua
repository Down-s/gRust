function GM:Move(pl, mv)
    if not pl:CanSprint() then mv:SetMaxClientSpeed(pl:GetWalkSpeed()) end
...
function PLAYER:OnReady()
    ...
    if SERVER then
        if self.Belt then self.Belt:SyncAll() end
        if self.Inventory then self.Inventory:SyncAll() end
        if self.Attire then self.Attire:SyncAll() end
    end
end
...
function PLAYER:OnInventoryAttached(inventory)
    if inventory:GetSize() == 6 then
        ...
        self.Belt.OnUpdated = function(belt) hook.Run("OnPlayerBeltUpdated", self) end
    elseif inventory:GetSize() == 7 then
        ...
        self.Attire.OnUpdated = function(attire) hook.Run("OnPlayerAttireUpdated", self) end
    else
        self.Inventory = inventory
        hook.Run("OnPlayerInventoryAttached", self, inventory)
        self.Inventory.OnUpdated = function(inventory) hook.Run("OnPlayerInventoryUpdated", self) end
    end
end

function PLAYER:ItemCount(item)
    local count = 0
    if self.Belt then count = count + self.Belt:ItemCount(item) end
    if self.Inventory then count = count + self.Inventory:ItemCount(item) end
    if self.Attire then count = count + self.Attire:ItemCount(item) end
    return count
end
