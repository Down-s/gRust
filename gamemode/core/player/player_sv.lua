...
local JumpPower = 190
util.AddNetworkString("gRust.ReadyStatus")
function GM:PlayerSpawn(pl)
    --
    -- Setup
    --
    net.Start("gRust.ReadyStatus")
    net.Send(pl)
...
hook.Add("InitPostEntity", "gRust.LoadPlayerSpawns", function()
    timer.Create("IDSpawnPoint", 1, 0, function()
        -- 
        gRust.SpawnPoints = ents.FindByClass("info_player_start")
        if #gRust.SpawnPoints > 0 then
            timer.Remove("IDSpawnPoint")
            print("#" .. #gRust.SpawnPoints .. " Spawn points loaded")
        end
    end)
end)

function GM:PlayerSelectSpawn(pl, transition)
    if #gRust.SpawnPoints < 1 then return gRust.SpawnPoints[1] end
    gRust.NextSpawnPoint = (gRust.NextSpawnPoint or 0) + 1
    if gRust.NextSpawnPoint > #gRust.SpawnPoints then gRust.NextSpawnPoint = 1 end
    return gRust.SpawnPoints[gRust.NextSpawnPoint]
end

function GM:PlayerInitialSpawn(pl)
    pl:RegisterSyncVar("Health", SyncVar.Float or math.random(50, 67))
end
...
hook.Add("OnPlayerBeltUpdated", "gRust.PlayerBeltUpdated", function(pl)
    ...
    for i = 1, belt:GetSize() do
        ...
        if item and item:IsWeapon() then
            ...
            weps[wepClass] = true
            if not ent:HasWeapon(wepClass) then ent:Give(wepClass) end
        end
    end
    ...
    for i = 2, #weapons do
        local v = weapons[i]
        if not weps[v:GetClass()] then ent:StripWeapon(v:GetClass()) end
    end
end)
...
function GM:GetFallDamage(pl, speed)
    pl:AddHealth(speed * 0.05)
    if pl:Alive() and pl:Health() <= 0 then pl:Kill() end
    --return speed * 0.05
end
...
net.Receive("gRust.PlayerReady", function(len, pl)
    ...
    for k, v in ipairs(ents.FindByClass("rust_sleepingplayer")) do
        ...
        if owner == pl:SteamID() then
            ...
            pl:SetPos(v:GetPos())
            local health = v.PlayerHealth or math.random(50, 67)
    ...
    if not foundSleepingPlayer then GAMEMODE:PlayerLoadout(pl) end
end)
...
function PLAYER:GetDefaultPlayerModel()
    ...
    if not model then model = PlayerModels[math.random(#PlayerModels)] end
    return model
end
...
--[[function PLAYER:SetHealth(amount)
    if self:Alive() and self:Health() <= 0 then self:Kill() end
...
function GM:EntityTakeDamage(ent, dmg)
    if ent:IsPlayer() then
        ...
        --if ent:IsInSafeZone() and not ent:IsCombatBlocked() then return true end
        --if dmg:GetDamageType() == DMG_CRUSH then return true end
...
function GM:PlayerCanHearPlayersVoice(listener, talker)
    if listener:GetPos():DistToSqr(talker:GetPos()) <= 250000 then return true, true end
end

function GM:PlayerCanSeePlayersChat(text, teamOnly, listener, speaker)
    if teamOnly then return listener.TeamID == speaker.TeamID end
    return true
end
...
function PLAYER:AddItem(item, reason)
    if isstring(item) then item = gRust.CreateItem(item) end
    ...
    if register:IsWeapon() or register:IsDeployable() then
        ...
        if rem then
            ...
            if rem then rem = self.Inventory:InsertItem(rem) end
        end
    else
        rem = self.Belt:InsertItem(item, true)
        if rem then
            rem = self.Inventory:InsertItem(rem)
            if rem then rem = self.Belt:InsertItem(rem) end
        end
    end
    ...
    if rem then
        ...
        timer.Simple(0, function()
            local phys = ent:GetPhysicsObject()
            if IsValid(phys) then phys:SetVelocity(self:GetAimVector() * 100) end
        end)
    end
...
function PLAYER:RemoveItem(item, quantity, reason)
    ...
    if quantity > 0 and self.Inventory:ItemCount(item) > 0 then self.Inventory:RemoveItem(item, quantity) end
    if quantity > 0 and self.Attire:ItemCount(item) > 0 then self.Attire:RemoveItem(item, quantity) end
    if reason ~= ITEM_GENERIC then
